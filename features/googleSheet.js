...
export async function updateGoogleSheet() {
  ...
  try {
    ...
    const partyFiles = allFiles.filter(
      (f) => f.startsWith("party") && f.endsWith(".json")
    );
    ...
    for (const fileName of partyFiles) {
      ...
      if (!existingSheets.find((s) => s.title === sheetTitle)) {
        console.log(
          `[GSheet] Tab "${sheetTitle}" tidak ditemukan, akan dibuat...`
        );
        createRequests.push({
          addSheet: {
    ...
    if (createRequests.length > 0) {
      ...
      console.log(
        `[GSheet] ${createRequests.length} tab baru telah dibuat. Menjalankan ulang untuk mengisi data...`
      );
    ...
    for (const fileName of partyFiles) {
      ...
      if (!sheetProps) {
        console.warn(
          `[GSheet] Aneh, tab ${sheetTitle} masih tidak ditemukan. Skipping.`
        );
        continue;
      }
      ...
      const mainPlayers = Object.entries(data).filter(
        ([_, info]) => info.tipe === "main"
      );
      const subsPlayers = Object.entries(data).filter(
        ([_, info]) => info.tipe === "subs"
      );
      const exSubsPlayers = Object.entries(data).filter(
        ([_, info]) => info.tipe === "exsubs"
      );
      const otherPlayers = Object.entries(data).filter(
        ([_, info]) => !["main", "subs", "exsubs"].includes(info.tipe)
      );
      ...
      const createRowData = (name, info) => ({
        values: [
          ...
          {
            userEnteredValue: {
              stringValue: info.status === "❌" ? info.alasan || "" : "",
            },
          },
        ],
      });

      mainPlayers.forEach(([name, info]) =>
        rows.push(createRowData(name, info))
      );
      subsPlayers.forEach(([name, info]) =>
        rows.push(createRowData(name, info))
      );
      exSubsPlayers.forEach(([name, info]) =>
        rows.push(createRowData(name, info))
      );
      otherPlayers.forEach(([name, info]) =>
        rows.push(createRowData(name, info))
      );
    ...
    if (updateRequests.length > 0) {
      ...
      console.log(
        "[GSheet] Tidak ada file party ditemukan. Update dibatalkan."
      );
    }
  } catch (err) {
    ...
    if (err.message && err.message.includes("permission")) {
      ...
      console.error(
        `[GSheet] PASTIKAN Anda sudah membagikan (Share) Sheet Anda ke email: ${email}`
      );
    }
  }
}
