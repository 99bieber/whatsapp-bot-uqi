...
export async function startDiscordBot(waSocket) {
  ...
  if (!DISCORD_TOKEN || !DISCORD_CHANNEL_ID || !WA_GROUP_ID.includes("@g.us")) {
    console.error(
      "❌ [Discord] Gagal memulai: DISCORD_TOKEN, DISCORD_CHANNEL_ID, atau WA_GROUP_ID belum diisi dengan benar di config.js."
    );
    return;
  }
  ...
  discordClient.on("ready", () => {
    console.log(
      `✅ [Discord] Bot Discord terhubung sebagai ${discordClient.user.tag}!`
    );
  ...
  discordClient.on("messageCreate", async (message) => {
    ...
    console.log(
      `[Discord] Mendeteksi pesan bot di #${message.channel.name}. Memproses...`
    );

    if (message.embeds.length === 0) {
      console.warn(
        "[Discord] Pesan bot terdeteksi tapi tidak ada embed. Mengirim teks biasa."
      );
      if (message.content) {
        await waSocket.sendMessage(WA_GROUP_ID, {
          text: `[Notif Pancingan 🎣]\n\n${message.content}`,
        });
      }
      return;
    }
    ...
    try {
      if (imageUrl) {
        ...
        console.log(
          `[Discord->WA] Sukses mengirim notifikasi ikan DENGAN GAMBAR ke grup WA!`
        );
      } else {
        await waSocket.sendMessage(WA_GROUP_ID, { text: waCaption.trim() });
        console.log(
          `[Discord->WA] Sukses mengirim notifikasi ikan (tanpa gambar) ke grup WA!`
        );
      }
    } catch (e) {
  ...
  try {
    ...
    console.error(
      "❌ [Discord] GAGAL LOGIN. Apakah Token Anda sudah benar?",
      e.message
    );
  }
}
