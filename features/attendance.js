...
export function updateAttendance(party, name, status, alasan) {
  ...
  const foundKey = dataKeys.find(
    (key) => key.toLowerCase() === name.toLowerCase()
  );
  if (!foundKey) {
    console.log(
      `⚠️ Gagal update: Nama "${name}" tidak ditemukan di ${party}.json`
    );
    return false;
  }
...
export async function runDailyAbsenCheck(sock) {
  ...
  try {
    ...
    console.log(
      "[CRON 17:07] Gagal membaca file JSON (activeParties/resetState).",
      e
    );
    return;
  }
  ...
  for (const [chatId, partyName] of Object.entries(activeParties)) {
    if (resetState[partyName]) {
      console.log(
        `[CRON 17:07] Party ${partyName} di-skip karena baru direset.`
      );
      continue;
    }
    ...
    for (const [playerName, info] of Object.entries(data)) {
      if (!info.status || info.status === "") {
        ...
        isUpdated = true;
        console.log(
          `[CRON 17:07] ${playerName} di ${partyName} ditandai ❌ (Tidak Ada Kabar).`
        );
      }
    }
    if (isUpdated) {
      ...
      console.log(
        `[CRON 17:07] Tidak ada update untuk ${partyName}, semua sudah absen.`
      );
    }
    console.log(
      `[CRON 17:07] Mengirim laporan absen final ${partyName} ke grup ${chatId}...`
    );
    try {
  ...
  updateGoogleSheet().catch((e) =>
    console.error("[GSheet] Gagal update:", e.message)
  );
}
