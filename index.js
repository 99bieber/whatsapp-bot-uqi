...
function initializeFiles() {
    ...
    if (!fs.existsSync(RESET_STATE_FILE)) fs.writeFileSync(RESET_STATE_FILE, JSON.stringify({}))
    if (!fs.existsSync(ADMIN_FILE)) fs.writeFileSync(ADMIN_FILE, JSON.stringify({ "6281945920003": "Owner" }))
    if (!fs.existsSync(ACTIVE_PARTIES_FILE)) fs.writeFileSync(ACTIVE_PARTIES_FILE, JSON.stringify({}))
}
...
async function connectToWhatsApp() {
    ...
    sock.ev.on('connection.update', (update) => {
            ...
            if (connection === 'open') {
                ...
                if (!isSchedulerRunning) {
                    ...
                    console.log(`ℹ️ Scheduler absen otomatis (Alarm ${CRON_SCHEDULE_ABSEN}) non-aktif. Hanya bisa via /startDailyChecking.`)
                    ...
                    console.log(`🔥 Menjalankan Ping Keep-Alive (${CRON_SCHEDULE_PING})...`)
                    cron.schedule(CRON_SCHEDULE_PING, async() => {
                        try {
                ...
                console.log(`❌ Koneksi terputus. ${shouldReconnect ? 'Menyambung ulang...' : 'Scan ulang QR diperlukan.'}`)
    ...
    sock.ev.on('messages.upsert', async({ messages, type }) => {
            ...
            try {
                ...
                try {
                    await sock.sendMessage(chatId, { text: `⚠️ Bot mengalami error: ${e.message}` })
                } catch (err) {
